...
tear_value = [0, 0, 0, 0, 0, 0]
count = 0
plot_res = 3  # Higher number means faster plotting, less resolution
ForceInput_x_T = [[] for _ in range(6)]  # Assuming 6 data points
...
def start():
    ...
    ax.legend(loc='upper right')
    ax.set_title('Force Input x T')
    ax.set_xlim(0, 100)  # Adjust this based on expected count range
    ax.set_ylim(-100, 100)  # Adjust this based on expected data range
...
def update(frame):
    for i, line in enumerate(lines):
        line.set_data(range(count), ForceInput_x_T[i])
    return lines
...
def readSerial_writeTOcsv():
    ...
    ForceInput_x_T_values = np.sum(np.multiply(ForceInput[:, np.newaxis], T), axis=1) + tear_value
    count += 1

    for i in range(6):
        ForceInput_x_T[i].append(ForceInput_x_T_values[i])

    string_to_write = f"{count},{time.time()},{','.join(map(str, ForceInput_x_T_values))}"
    file.write(string_to_write + "\n")

def tear():
    global tear_value 
    average_ForceInput_x_T = np.zeros_like(tear_value)
    for _ in range(10):  # Amount to average 
        average_ForceInput_x_T = (average_ForceInput_x_T + np.array(ForceInput_x_T)[:, -1]) / 2
